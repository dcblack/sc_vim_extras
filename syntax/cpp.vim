...
syntax keyword stlType       array vector list map set multiset multimap bitset deque forward_list unordered_map unordered_set
syntax keyword stlType       ios_base openmode trunc app ate in out future
syntax keyword stlType       iterator const_iterator pair std tr1 string numeric_limits
syntax keyword stlType       stringstream ostringstream istringstream ofstream ifstream ostream istream
syntax keyword stlType       initializer_list tie tuple make_tuple thread future promise chrono mutex lock_guard
...
syntax keyword stlType       shuffle_order_engine independent_bits_engine discard_block_engine
...
syntax keyword stdType       normal_distribution lognormal_distribution poisson_distribution exponential_distribution
...
syntax keyword cppStatement  const_cast reinterpret_cast static_cast dynamic_cast
syntax keyword cppStatement  constexpr static_assert decltype
syntax keyword cppStatement  default delete override final noexcept
syntax keyword cppType       int64 uint64 nullptr_t
syntax keyword cppType       int8_t int16_t int32_t int64_t uint8_t uint16_t uint32_t uint64_t
...
syntax keyword cppDefines    eq ne forever repeat
...
syntax keyword cppio         uppercase showbase hex setprecision fixed scientific floatfield dec oct flush contained containedin=iomanip
...
syntax match   iomanip       /<< *std::\w\+\>/ contains=cppio,scStatement,std,greensocs,stdlib,stlMethod

syntax match   eaFeature     /hldw::\w\+/
syntax match   eaFeature     /HLDW_\w\+/
syntax keyword eaFeature     forever repeat foreach
...
syntax match   scReports     /\<REPORT(\(INFO\|WARNING\|ERROR\|FATAL\),/
...
syntax keyword eaReports     EA_ASSERT EA_REPORT_INFO EA_REPORT_DEBUG EA_REPORT_LOGONLY
syntax keyword eaReports     EA_REPORT_WARNING EA_REPORT_ERROR EA_REPORT_FATAL
syntax keyword eaReports     SET_REPORT_ID ASSERT PROTOCOL
syntax keyword eaReports     REPORT_INFO REPORT_INFO_VERB REPORT_DEBUG REPORT_LOGONLY
syntax keyword eaReports     REPORT_DATA REPORT_NOTE REPORT_ALERT
syntax keyword eaReports     REPORT_WARNING REPORT_ERROR REPORT_FATAL
syntax keyword eaReports     REPORTF_INFO  REPORTF_WARNING
syntax keyword eaReports     REPORTF_ERROR REPORTF_FATAL
syntax keyword eaReports     EA_START EA_FINISH
syntax keyword eaReports     EA_EXPECT_WARNING EA_EXPECT_ERROR EA_EXPECT_FATAL

syntax keyword eaReports     ESLX_ASSERT ESLX_REPORT_INFO ESLX_REPORT_DEBUG ESLX_REPORT_LOGONLY
syntax keyword eaReports     ESLX_REPORT_WARNING ESLX_REPORT_ERROR ESLX_REPORT_FATAL
syntax keyword eaReports     ESLX_START ESLX_FINISH
syntax keyword eaReports     ESLX_EXPECT_WARNING ESLX_EXPECT_ERROR ESLX_EXPECT_FATAL

syntax keyword eaReports     XEDA_ASSERT XEDA_REPORT_INFO XEDA_REPORT_DEBUG XEDA_REPORT_LOGONLY
syntax keyword eaReports     XEDA_REPORT_WARNING XEDA_REPORT_ERROR XEDA_REPORT_FATAL
syntax keyword eaReports     XEDA_START XEDA_FINISH
syntax keyword eaReports     XEDA_EXPECT_WARNING XEDA_EXPECT_ERROR XEDA_EXPECT_FATAL
syntax keyword eaReports     THROW_INFO THROW_WARNING THROW_ERROR THROW_FATAL DEBUG_ENTRY DEBUG_EXIT NOT_YET_IMPLEMENTED UNDER_CONSTRUCTION

syntax keyword eaStreams     sc_report_info sc_report_warning sc_report_error sc_report_fatal sc_report_contd sc_report_end
...
syntax keyword scRptCfg      get_msg_type get_severity get_process_name get_msg get_time get_file_name
...
syntax keyword eaStreams     sys_info_stream sys_warning_stream sys_error_stream sys_fatal_stream
...
highlight sky     ctermfg=DarkBlue  ctermbg=none      guifg=#00bfff   guibg=White gui=bold        term=bold
highlight gold    ctermfg=Yellow    ctermbg=none      guifg=#d0c000   guibg=White gui=bold        term=bold
...
" Dark:   Darkred Darkgreen Darkblue Darkmagenta Darkcynan Darkyellow
  if &background == "dark"
    highlight Emphasize term=bold,underline cterm=NONE ctermfg=LightBlue ctermbg=NONE     gui=bold guifg=#2e8b57 guibg=NONE
    highlight Warn      term=bold,underline cterm=NONE ctermfg=LightRed  ctermbg=NONE     gui=bold guifg=#2e8b57 guibg=NONE
    highlight Directive	term=bold           cterm=NONE ctermfg=Magenta   ctermbg=NONE     gui=bold guifg=#ffa0a0 guibg=NONE
    highlight Template  term=bold                      ctermfg=White     ctermbg=DarkBlue gui=bold guifg=Cyan    guibg=DarkRed
  else
    highlight Emphasize term=bold,underline cterm=NONE ctermfg=LightBlue ctermbg=NONE     gui=bold guifg=#2e8b57 guibg=NONE
    highlight Warn      term=bold,underline cterm=NONE ctermfg=LightRed  ctermbg=NONE     gui=bold guifg=#2e8b57 guibg=NONE
    highlight Directive	term=bold           cterm=NONE ctermfg=DarkRed   ctermbg=NONE     gui=bold guifg=Magenta guibg=NONE
...
: HiLink cppWarn                Red
: HiLink cppDefines             Statement
" HiLink scFeature              sky
: HiLink eaFeature              purple
: HiLink scStatement            sky
: HiLink scExceptions           scStatement
: HiLink scSpecial              Red
: HiLink scReports              Red
: HiLink eaReports              Red
